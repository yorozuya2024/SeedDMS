...
class SeedDMS_Preview_Previewer extends SeedDMS_Preview_Base {
	...
	public function createPreview($object, $width=0, &$new=false) { /* {{{ */
		...
		if($width == 0)
			...
			$width = intval($width);
		$document = $object->getDocument();
		$storage = $document->_dms->getStorage();
		if($storage) {
			$file = tempnam(sys_get_temp_dir(), "preview-");
			file_put_contents($file, $object->content());
		} else {
			$file = $document->_dms->contentDir.$object->getPath();
		}
		$target = $this->getFileName($object, $width);
		$ret = $this->createRawPreview($file, $document->getDir(), $object->getMimeType(), $width, $target, $new);
		if($storage)
			unlink($file);
		return $ret;
	} /* }}} */
