...
//						return false;
	 ...
	 * @return string sql code
	 */
	public function getCurrentDatetime($offset=0) { /* {{{ */
		switch ($this->_driver) {
		case 'mysql':
			if($offset)
				return "CURRENT_TIMESTAMP + INTERVAL ".((int) $offset)." SECOND";
			else
				return "CURRENT_TIMESTAMP";
			break;
		case 'sqlite':
			if($offset)
				return "datetime('now', 'localtime', '".((int) $offset)." seconds')";
			else
				return "datetime('now', 'localtime')";
			break;
		case 'pgsql':
			if($offset)
				return "now() + INTERVAL ".((int) $offset)." SECOND";
			else
				return "now()";
			break;
		}
