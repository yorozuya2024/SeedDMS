...
//							$tasks['review'][] = array('id'=>$res['id'], 'name'=>$res['name']);
	...
	function menuTasks() { /* {{{ */
		...
		foreach($tasks['revision'] as $t) {
			...
			$subitems[] = array('label'=>htmlspecialchars($doc->getName()).' ('.$stat[1].'/'.($stat['0']+$stat['1']).')', 'link'=>"../out/out.ViewDocument.php?documentid=".$doc->getID()."&currenttab=revision", 'class'=>"table-row-document", 'rel'=>"document_".$doc->getID());
		}
